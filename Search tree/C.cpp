...
struct Node {
    int size = 1, p = rand() * rand(), data, zero;
...
struct Treap {
    ...
    void update_zero(Node* t) {
        t->zero = get_zero(t->left) || get_zero(t->right) || (t->data == 0);
    }
    ...
    void split(Node* t, int key, Node* &l, Node* &r) {
        ...
        if (get_size(t->left) < key) {
            split(t->right, key - get_size(t->left) - 1, t->right, r);
            l = t;
        } else {
    ...
    pair<Node*, int> search_null(Node* t) {
        Node* cur = t;
        int ind = get_size(cur->left);

        while (get_zero(cur)) {
            if (cur->left != nullptr && get_zero(cur->left)) {
                cur = cur->left;
                ind = ind - get_size(cur->right) - 1;
            } else if (get_data(cur) == 0) {
                break;
            } else {
                cur = cur->right;
                ind += get_size(cur->left);
                ++ind;
            }
        }

        return make_pair(cur, ind);
    }

    void to_vector(Node* node, vector<int> &a) {
        if (node) {
            to_vector(node->left, a);
            a.push_back(get_data(node));
            to_vector(node->right, a);
        }
    }
    ...
    void build(int n) {
        for (int i = 0; i < n; ++i) root = merge(new Node, root);
    }
    ...
    void insert(int ind) {
        ...
        split(root, ind, l, r);
        auto [z, i] = search_null(r);
        if (z != nullptr && z->data == 0) r = remove(r, i);
