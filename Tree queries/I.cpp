...
public:
	struct Vertex {
		...
		bool revert;
		int size;
		long long value, result, delta;

		int vertexes_by_light, vertexes_by_light_path_ans;

		Vertex():
			left(-1), right(-1), parent(-1), revert(),
			size(1), value(), result(), delta(),
			vertexes_by_light(0), vertexes_by_light_path_ans(1) {}
	};
	...
	long long getResult(int x) const {
		return t[x].result + t[x].delta * t[x].size;
	}

	int getVertexesByLight(int x) const {
		return t[x].vertexes_by_light;
	}

	int getVertexesByLightPathAns(int x) const {
		return t[x].vertexes_by_light_path_ans;
	}
	...
	void keep(int x) {
		...
		t[x].vertexes_by_light_path_ans = t[x].vertexes_by_light + 1;
		if (t[x].left != -1) t[x].vertexes_by_light_path_ans += t[t[x].left].vertexes_by_light_path_ans;
		if (t[x].right != -1) t[x].vertexes_by_light_path_ans += t[t[x].right].vertexes_by_light_path_ans;
	}
	...
	int expose(int x) {
		...
		for (int y = x; y != -1; y = t[y].parent) {
			splay(y);
			if (t[y].right != -1) {
				t[y].vertexes_by_light += t[t[y].right].vertexes_by_light_path_ans;
			}
			if (c != -1) {
				t[y].vertexes_by_light -= t[c].vertexes_by_light_path_ans;
			}
			t[y].right = c;

			t[y].vertexes_by_light_path_ans = t[y].vertexes_by_light + 1;
			if (t[y].left != -1) t[y].vertexes_by_light_path_ans += t[t[y].left].vertexes_by_light_path_ans;
			if (t[y].right != -1) t[y].vertexes_by_light_path_ans += t[t[y].right].vertexes_by_light_path_ans;
	...
	void link(int x, int y) {
		...
		t[x].parent = y;
		t[y].vertexes_by_light += t[x].vertexes_by_light_path_ans;
	...
	void cut(int x) {
		...
		t[t[x].left].vertexes_by_light_path_ans -= t[x].vertexes_by_light_path_ans;
...
signed main() {
	...
	for (int req = 0; req < m; ++req) {
		...
		if (req_type == "link") {
			...
			cin >> v; --v;
			cout << tree.getVertexesByLightPathAns(tree.root(v)) << endl;
		}
	}
}
