...
void dsu_union(vector<dsuNode> &dsu, int a, int b) {
    ...
    if (a != b) {
        ...
        dsu[b].parent = a;
        dsu[a].min_elem = min(dsu[a].min_elem, dsu[b].min_elem);
        dsu[a].max_elem = max(dsu[a].max_elem, dsu[b].max_elem);
        dsu[a].count_elem = dsu[a].count_elem + dsu[b].count_elem;
    }
}
...
signed main() {
    ...
    while (cin >> type) {
        if (type.front() == 'u') {
            cin >> a >> b;
            dsu_union(dsu, a - 1, b - 1);

        } else {
            cin >> a;
            dsuNode result1 = dsu[dsu_get(dsu, a - 1)];
            cout << result1.min_elem + 1 << ' ' << result1.max_elem + 1 << ' ' << result1.count_elem << endl;
        }
    }
}
