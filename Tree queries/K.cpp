...
void count(vector<vector<int>> &graph, vector<int> &centroids, vector<int> &w, int u, int pred) {
    w[u] = 1;
    for (int v : graph[u]) {
        if (v == pred || centroids[v] != -2) continue;
        count(graph, centroids, w, v, u);
        w[u] += w[v];
    }
}

int find_centroid(vector<vector<int>> &graph, vector<int> &centroids, vector<int> &w, int u, int pred, int s) {
    bool flag = true;
    while (flag) {
        flag = false;
        for (int v : graph[u]) {
...
void find_dist(vector<vector<int>> &graph, int cent, set<int> &cent_children, vector<vector<pair<int, int>>> &data, int u, int pred = -1, int distance = 0) {
    if (cent_children.find(u) == cent_children.end()) return;

    data[cent].emplace_back(distance, u);

    for (int next : graph[u]) {
        if (next != pred) find_dist(graph, cent, cent_children, data, next, u, distance + 1);
    }
}

pair<int, set<int>> decompose(
    vector<vector<int>> &graph, vector<int> &centroids, vector<int> &w,
    vector<vector<pair<int, int>>> &data,
    int u = 0, int pred = -1
) {
    count(graph, centroids, w, u, pred);
    u = find_centroid(graph, centroids, w, u, pred, w[u]);
    centroids[u] = pred;
    ...
    for (int v : graph[u]) {
        if (centroids[v] != -2) continue;
        auto p = decompose(graph, centroids, w, data, v, u);
        v = p.first;
        for (auto &item : p.second) {
            cur_data.insert(item);
        }
    }
    data[u].reserve(cur_data.size() + 1);  // Memory error otherwise
    find_dist(graph, u, cur_data, data, u);
    sort(all(data[u]));
    return make_pair(u, cur_data);
}
...
signed main() {
    ...
    vector<int> centroids(n, -2);
    vector<int> w(n);
    vector<vector<pair<int, int>>> data(n);
    decompose(graph, centroids, w, data);

    vector<map<int, int>> data_map(n);
    for (int i = 0; i < n; ++i) {
        for (auto &[distance, u] : data[i]) {
            data_map[i][u] = distance;
        }
    }

    vector<vector<int>> data_min(n);
    for (int i = 0; i < n; ++i) {
        int min_v = INT_MAX;
        for (auto &[distance, u] : data[i]) {
            min_v = min(min_v, u);
            data_min[i].push_back(min_v);
        }
    }

    int ans, cur_centroid, dist2centroid, left, right, middle;
    for (int req = 0; req < m; ++req) {
        cin >> v >> d; --v;

        ans = cur_centroid = v;
        while (cur_centroid != -1) {
            dist2centroid = data_map[cur_centroid][v];

            left = -1;
            right = data[cur_centroid].size();
            while (right - left > 1) {
                middle = (left + right) / 2;
                if (data[cur_centroid][middle].first <= d - dist2centroid) {
                    left = middle;
                } else {
                    right = middle;
                }
            ...
            if (left != -1) ans = min(ans, data_min[cur_centroid][left]);

            cur_centroid = centroids[cur_centroid];
        }
