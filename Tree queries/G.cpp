...
void build_lca_dp(vector<vector<int>> &graph, vector<int> &depth, vector<vector<int>> &lca_dp, int max_i, int v = 0, int pred = 0) {
    if (pred != v) depth[v] = depth[pred] + 1;

    lca_dp[v][0] = pred;
    for (int i = 1; i <= max_i; ++i) lca_dp[v][i] = lca_dp[lca_dp[v][i - 1]][i - 1];

    for (int next : graph[v]) {
        if (next != pred) build_lca_dp(graph, depth, lca_dp, max_i, next, v);
    }
}

int lca(vector<int> &depth, vector<vector<int>> &lca_dp, int max_i, int u, int v) {
    if (depth[v] > depth[u]) swap(v, u);

    for (int i = max_i; i >= 0; --i) {
        if (depth[lca_dp[u][i]] >= depth[v]) u = lca_dp[u][i];
    }

    if (v == u) return v;

    for (int i = max_i; i >= 0; --i) {
        if (lca_dp[v][i] != lca_dp[u][i]) {
            v = lca_dp[v][i];
            u = lca_dp[u][i];
        }
    }
...
void segtree_update(vector<long long> &tree, int v, int tl, int tr, int l, int r, int value) {
    ...
    if (l == tl && tr == r) {
        ...
        int tm = (tl + tr) / 2;
        segtree_update(tree, v * 2, tl, tm, l, min(r, tm), value);
        segtree_update(tree, v * 2 + 1, tm + 1, tr, max(l, tm + 1), r, value);
    }
}

int count_heavy_child(vector<vector<int>> &graph, vector<int> &heavy_child, int v = 0, int pred = -1) {
    int max_child = -1, max_child_val = 0, size = 1, child_size;

    for (int child : graph[v]) {
        if (child != pred) {
            int child_size = count_heavy_child(graph, heavy_child, child, v);
            size += child_size;
            if (child_size > max_child_val) {
                max_child_val = child_size;
                max_child = child;
            }
        }
    }
    heavy_child[v] = max_child;
    return size;
}

int create_mapping(
    vector<vector<int>> &graph, vector<int> &heavy_child, vector<int> &mapping, vector<int> &head, vector<int> &pred,
    int v = 0, int num = 0, int cur_pred = -1, int cur_head = 0
) {
    mapping[v] = num++;
    head[mapping[v]] = mapping[cur_head];  // Warning! mapping[cur_head] can be initialized on the line above (v = cur_head)
    pred[mapping[v]] = mapping[cur_pred];

    if (heavy_child[v] != -1) num = create_mapping(graph, heavy_child, mapping, head, pred, heavy_child[v], num, v, cur_head);

    for (int child : graph[v]) {
        if (child != cur_pred && child != heavy_child[v]) {
            num = create_mapping(graph, heavy_child, mapping, head, pred, child, num, v, child);
        }
    }
    return num;
}

void add(vector<long long> &segtree, int n, vector<int> &head, vector<int> &pred, int v, int u, int value) {
    while (true) {
        int cur_head = head[v];
        ...
        segtree_update(segtree, 1, 0, n, cur_head, v, value);
        v = cur_head;
...
signed main() {
    ...
    vector<int> depth(n);
    vector<vector<int>> lca_dp(n);
    int max_i = 1;
    while ((1 << max_i) <= n) ++max_i;
    for (int i = 0; i < n; ++i) lca_dp[i].resize(max_i + 1);
    build_lca_dp(graph, depth, lca_dp, max_i);

    vector<int> heavy_child(n);
    count_heavy_child(graph, heavy_child);

    vector<int> mapping(n), reverse_mapping(n), head(n), pred(n);
    create_mapping(graph, heavy_child, mapping, head, pred);
    for (int i = 0; i < n; ++i) reverse_mapping[mapping[i]] = i;
    ...
    for (int req = 0; req < m; ++req) {
        ...
        if (req_type == '+') {
            cin >> v >> u >> d; --v; --u;
            int vu_lca = lca(depth, lca_dp, max_i, u, v);

            add(segtree, n, head, pred, mapping[v], mapping[vu_lca], d);
            add(segtree, n, head, pred, mapping[u], mapping[vu_lca], d);
            add(segtree, n, head, pred, mapping[vu_lca], mapping[vu_lca], -d);

        } else {
            cin >> v; --v;
            cout << segtree_get(segtree, 1, 0, n, mapping[v]) << endl;
        }
    }
}
