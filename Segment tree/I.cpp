...
vector<int> ID_MATRIX{1, 0, 0, 1};

vector<int> matrix_multiply(vector<int> &matrix1, vector<int> &matrix2) {
    vector<int> result{
        (matrix1[0] * matrix2[0] + matrix1[1] * matrix2[2]) % MOD,
        (matrix1[0] * matrix2[1] + matrix1[1] * matrix2[3]) % MOD,
        (matrix1[2] * matrix2[0] + matrix1[3] * matrix2[2]) % MOD,
        (matrix1[2] * matrix2[1] + matrix1[3] * matrix2[3]) % MOD
    };
    return result;
}

void build(vector<vector<int>> &tree, vector<vector<int>> &src, int v, int tl, int tr) {
    if (tl == tr) {
        tree[v] = src[tl];
    } else {
        int tm = (tl + tr) / 2;
        ...
        tree[v] = matrix_multiply(tree[v * 2], tree[v * 2 + 1]);
    }
}

vector<int> product(vector<vector<int>> &tree, int v, int tl, int tr, int l, int r) {
    if (l > r) return ID_MATRIX;

    if (l == tl && r == tr) return tree[v];

    int tm = (tl + tr) / 2;

    vector<int> left = product(tree, v * 2, tl, tm, l, min(r, tm)),
                right = product(tree, v * 2 + 1, tm + 1, tr, max(l, tm + 1), r);
...
signed main() {
    ...
    int R, n, m;
    cin >> R >> n >> m;
    vector<vector<int>> a(n, ID_MATRIX);
    for (int i = 0; i < n; ++i) {
    ...
    for (int req = 0; req < m; ++req) {
        cin >> l >> r;
        vector<int> result = product(tree, 1, 0, n - 1, l - 1, r - 1);
        cout << result[0] << ' ' << result[1] << endl
             << result[2] << ' ' << result[3] << endl << endl;
    }
}
