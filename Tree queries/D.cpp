...
int warp_get(vector<int> &warp, int v) {
    if (v == warp[v]) return v;
    return warp[v] = warp_get(warp, warp[v]);
}

signed main() {
    ios_base::sync_with_stdio(false);cin.tie(NULL);cout.tie(NULL);

    vector<int> depth(1), warp(1);
    vector<vector<int>> dp(1);
    int steps = 1;
    while ((1 << steps) <= 200'000) ++steps;
    dp[0].resize(++steps);
    ...
    for (int req = 0; req < M; ++req) {
        ...
        if (req_type == '+') {
            ...
            warp.push_back(warp.size());

            vector<int> dp_item(steps + 1);
            dp_item[0] = p;
            for (int i = 1; i <= steps; ++i) dp_item[i] = dp[dp_item[i - 1]][i - 1];
            dp.push_back(dp_item);
