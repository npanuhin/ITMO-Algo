...
void cube_add(vector<vector<vector<int>>> &cube, int x, int y, int z, int add) {
    for (int i = x; i < cube.size(); i |= (i + 1)) {
        for (int j = y; j < cube.size(); j |= (j + 1)) {
            for (int k = z; k < cube.size(); k |= (k + 1)) {
                cube[i][j][k] += add;
            }
        }
    }
...
int cube_get_in(vector<vector<vector<int>>> &cube, int x, int y, int z) {
    int ans = 0;
...
int cube_get(vector<vector<vector<int>>> &cube, int x1, int y1, int z1, int x2, int y2, int z2) {
     return cube_get_in(cube, x2, y2, z2) + cube_get_in(cube, x2, y1, z1) + cube_get_in(cube, x1, y2, z1) + cube_get_in(cube, x1, y1, z2)
          - cube_get_in(cube, x1, y1, z1) - cube_get_in(cube, x2, y2, z1) - cube_get_in(cube, x2, y1, z2) - cube_get_in(cube, x1, y2, z2);
}

signed main() {
    ios_base::sync_with_stdio(false);cin.tie(NULL);cout.tie(NULL);

    vector<int> cube_line(128);
    vector<vector<int>> cube_plane(128, cube_line);
    vector<vector<vector<int>>> cube(128, cube_plane);
    ...
    while (true) {
        ...
        if (req_type == '1') {
            cin >> x >> y >> z >> k;
            cube_add(cube, x, y, z, k);

        } else if (req_type == '2') {
            cin >> x1 >> y1 >> z1 >> x2 >> y2 >> z2;
            cout << cube_get(cube, x1 - 1, y1 - 1, z1 - 1, x2, y2, z2) << endl;
