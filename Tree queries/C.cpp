...
void build(vector<vector<pair<int, int>>> &graph, vector<int> &depth, vector<vector<pair<int, int>>> &dp, int max_i, int v, int p = 0, int c = INT_MAX) {
    ...
    for (auto next : graph[v]) build(graph, depth, dp, max_i, next.first, v, next.second);
}
...
signed main() {
    ...
    for (int req = 0; req < m; ++req) {
        cin >> u >> v;
        cout << min_lca(depth, dp, max_i, u - 1, v - 1) << endl;
    }
}
