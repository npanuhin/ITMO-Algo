...
pair<int, int> dsu_get(vector<dsuNode> &dsu, int v) {
    if (v == dsu[v].parent) return make_pair(v, 0);

    auto p = dsu_get(dsu, dsu[v].parent);

    return make_pair(dsu[v].parent = p.first, dsu[v].teamxp += p.second);
}

void dsu_union(vector<dsuNode> &dsu, int a, int b) {
    a = dsu_get(dsu, a).first;
    b = dsu_get(dsu, b).first;
    if (a != b) {
        if (dsu[a].rank < dsu[b].rank) swap(a, b);
        if (dsu[a].rank == dsu[b].rank) ++dsu[a].rank;

        dsu[b].parent = a;
        dsu[b].teamxp -= dsu[a].teamxp;
    }
}

void dsu_add(vector<dsuNode> &dsu, int a, int v) {
    dsu[dsu_get(dsu, a).first].teamxp += v;
}
...
signed main() {
    ...
    for (int req = 0; req < m; ++req) {
        ...
        } else if (type.front() == 'a') {
            cin >> x >> v;
            dsu_add(dsu, x - 1, v);

        } else {
            cin >> x;
            int root = dsu_get(dsu, x - 1).first;
            cout << dsu[x - 1].teamxp + ((root == x - 1) ? 0 : dsu[root].teamxp) << endl;
        }
    }
}
