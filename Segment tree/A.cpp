...
void build(vector<long long> &tree, vector<int> &src, int v, int tl, int tr) {
    if (tl == tr) {
        tree[v] = src[tl];
    } else {
        int tm = (tl + tr) / 2;
...
long long sum(vector<long long> &tree, int v, int tl, int tr, int l, int r) {
    if (l > r) return 0;
...
signed main() {
    ...
    for (int req = 0; req < m; ++req) {
        ...
        if (req_type == '1') {
            ...
            cin >> l >> r;
            cout << sum(tree, 1, 0, n - 1, l, r - 1) << endl;
        }
    }
}
