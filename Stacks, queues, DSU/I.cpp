...
//  If you're looking for the code to solve a specific problem, it starts down below, at about line 243
...
int dsu_get(vector<dsuNode> &dsu, int v) {
    if (v == dsu[v].parent) return v;
    return dsu[v].parent = dsu_get(dsu, dsu[v].parent);
}

void dsu_union(vector<dsuNode> &dsu, int a, int b) {
    a = dsu_get(dsu, a);
    b = dsu_get(dsu, b);
    if (a != b) {
        if (dsu[a].rank < dsu[b].rank) swap(a, b);
        if (dsu[a].rank == dsu[b].rank) ++dsu[a].rank;

        dsu[b].parent = a;

        if (dsu[a].is_full) {
            cnt_full -= dsu[a].count;
        } else {
            cnt_nfull -= dsu[a].count;
        }

        if (dsu[b].is_full) {
            cnt_full -= dsu[b].count;
        } else {
            cnt_nfull -= dsu[b].count;
        }

        dsu[a].count = dsu[a].count + dsu[b].count;
        dsu[a].is_full = (dsu[a].is_full || dsu[b].is_full);
...
signed main() {
    ...
    int n, m, q, x, y, x_root, y_root;
    char type;
    cin >> n >> m >> q;

    vector<dsuNode> dsu(n);
    for (int i = 0; i < n; ++i) dsu[i] = dsuNode(i);
    cnt_nfull = n; cnt_full = 0;


    for (int i = 0; i < m; ++i) {
        cin >> x >> y; --x; --y;
        x_root = dsu_get(dsu, x);
        y_root = dsu_get(dsu, y);

        if (x_root == y_root && !dsu[x_root].is_full) {
            dsu[x_root].is_full = true;
            cnt_nfull -= dsu[x_root].count;
            cnt_full += dsu[x_root].count;

        } else {
            dsu_union(dsu, x_root, y_root);
        }
    }
    ...
    for (int req = 0; req < q; ++req) {
        ...
        switch (type) {
            case '1':
                cin >> x >> y;
                x_root = dsu_get(dsu, x - 1);
                y_root = dsu_get(dsu, y - 1);

                if (x_root == y_root && !dsu[x_root].is_full) {
                    cout << "Yes" << endl;
                } else if (!dsu[x_root].is_full || !dsu[y_root].is_full) {
                    cout << "Yes" << endl;
                } else {
                    cout << "No" << endl;
                }
            ...
            case '3':
                cout << 2LL * cnt_nfull * cnt_full + (long long) cnt_nfull * (cnt_nfull - 1) << endl;
                continue;
        }
    }
}
