...
void build(vector<vector<int>> &graph, vector<int> &depth, vector<vector<int>> &dp, int max_i, int v, int p = 0) {
    if (p != v) depth[v] = depth[p] + 1;

    dp[v][0] = p;
    for (int i = 1; i <= max_i; ++i) dp[v][i] = dp[dp[v][i - 1]][i - 1];

    for (int next : graph[v]) {
        if (next != p) build(graph, depth, dp, max_i, next, v);
    }
}
...
signed main() {
    ...
    for (int i = 0; i < n; ++i) {
        cin >> cur_pred; --cur_pred;
        if (cur_pred < 0) root = cur_pred = i;
        graph[cur_pred].push_back(i);
    }
    ...
    build(graph, depth, dp, max_i, root);

    for (int i = 0; i < n; ++i) {
        cout << i + 1 << ": ";
        for (int j = 0; j < max_i; ++j) {
            if (j > 0 && dp[i][j] == dp[i][j - 1]) break;
            if (dp[i][j] == i) break;
            int dist = depth[i] - depth[dp[i][j]];
            if (dist & (dist - 1)) break;  // dist is not a power of two
