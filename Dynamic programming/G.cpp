...
void print_ans(vector<vector<int>> &dp, vector<vector<int>> &pred, string &s, int i, int j) {
    if (dp[i][j] == j - i + 1) {
        // pass
        ...
        cout << s[i];
        print_ans(dp, pred, s, i + 1, j - 1);
        cout << s[j];

    } else {
        print_ans(dp, pred, s, i, pred[i][j]);
        print_ans(dp, pred, s, pred[i][j] + 1, j);
    }
}
...
signed main() {
    ...
    vector<vector<int>> dp(n, tmp_v_int), pred(n, tmp_v_int);
    for (int i = 0; i < n; ++i) {
        dp[i][i] = 1;
    }
