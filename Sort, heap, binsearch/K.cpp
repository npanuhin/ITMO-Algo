...
int getParts(vector<int> &a, long long summ) {
    int parts = 0;
    long long cur_summ = 0;

    for (int i = 0; i < a.size(); ++i) {
        if (cur_summ + a[i] > summ) {
            cur_summ = 0;
            ++parts;
        }
        if (a[i] > summ) {
...
signed main() {
    ...
    long long left = LLONG_MAX / 2, right = -1, middle;
    while (left - right > 1) {
        middle = (left + right) / 2;
        if (getParts(a, middle) <= k) {
            left = middle;
        } else {
            right = middle;
        }
