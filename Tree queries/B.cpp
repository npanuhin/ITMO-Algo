...
void build(vector<vector<int>> &graph, vector<int> &depth, vector<vector<int>> &dp, int max_i, int v, int p = 0) {
    ...
    for (int next : graph[v]) build(graph, depth, dp, max_i, next, v);
}

int lca(vector<int> &depth, vector<vector<int>> &dp, int max_i, int u, int v) {
...
signed main() {
    ...
    int n, m, u, v, cur_pred;
    cin >> n;

    vector<vector<int>> graph(n);
    for (int i = 1; i < n; ++i) {
        cin >> cur_pred;
        graph[cur_pred - 1].push_back(i);
    }
    ...
    for (int req = 0; req < m; ++req) {
        cin >> u >> v;
        cout << lca(depth, dp, max_i, u - 1, v - 1) + 1 << endl;
    }
}
