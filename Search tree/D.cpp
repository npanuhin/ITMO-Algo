...
void split(Node *root, Node *&l, Node *&r, int x) {
    if (root == nullptr) {
        ...
        split(root->right, root->right, r, x);
        l = root;
    } else {
        split(root->left, l, root->left, x);
        r = root;
    }
    update_sum(l);
    update_sum(r);
...
void insert(int x) {
    ...
    split(treap, t1, t2, x);
    treap = merge(t1, new Node(x));
    treap = merge(treap, t2);
}

long long count_sum(long long l, long long r) {
    Node *t1, *t2, *t3;
    split(treap, t1, t2, l);
    split(t2, t2, t3, r + 1);
...
int main() {
    ...
    for (int req = 0; req < n; ++req) {
        ...
        switch (request) {
            case '+':
                cin >> i;
                if (last_sum != -1) i = (i + last_sum) % MOD;
                last_sum = -1;
                insert(i);
                break;
