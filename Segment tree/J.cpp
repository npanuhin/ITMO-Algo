...
int get_left_le(vector<int> &tree, int v, int tl, int tr, int l, int r, int x) {
    ...
    if (tl < l) {
        return min(
            get_left_le(tree, v * 2, tl, tm, l, min(r, tm), x),
            get_left_le(tree, v * 2 + 1, tm + 1, tr, max(l, tm + 1), r, x)
        );
    }

    if (tree[v * 2] <= x) {
        return get_left_le(tree, v * 2, tl, tm, l, min(r, tm), x);
    } else {
        return get_left_le(tree, v * 2 + 1, tm + 1, tr, max(l, tm + 1), r, x);
    }
}
...
signed main() {
    ...
    char req_type;
    int i, h, l, r, p, ans, left_le;
    for (int req = 0; req < m; ++req) {
        cin >> req_type;

        if (req_type == '1') {
            cin >> i >> h;
            update(tree, 1, 0, n - 1, i, h);

        } else {
            cin >> l >> r >> p;
            ans = 0;
            left_le = get_left_le(tree, 1, 0, n - 1, l, n - 1, p);
            while (left_le < r) {
                update(tree, 1, 0, n - 1, left_le, INT_MAX);
                ++ans;
                left_le = get_left_le(tree, 1, 0, n - 1, l, n - 1, p);
            }
            cout << ans << endl;
