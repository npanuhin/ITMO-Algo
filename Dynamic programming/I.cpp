    ...
    for (int j = 0; j < m; ++j) {
        for (int i = 0; i < n; ++i) {
            for (int mask = 0; mask < (1 << n); ++mask) {
                if ((mask & (1 << i)) > 0 || field[i][j]) {
                    d[j][i + 1][(mask | (1 << i)) - (1 << i)] += d[j][i][mask];
                } else {
                    if (!field[i][j + 1]) {
                        d[j][i + 1][mask + (1 << i)] += d[j][i][mask];
                    }
                    if ((mask & (1 << (i + 1))) == 0 && i < n - 1 && !field[i + 1][j]) {
                        d[j][i + 1][mask + (1 << (i + 1))] += d[j][i][mask];
                    }
                }
            }
        }
        for (int mask = 0; mask < (1 << n); ++mask) {
            d[j + 1][0][mask] = d[j][n][mask];
        }
    }
