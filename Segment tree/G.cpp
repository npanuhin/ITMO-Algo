...
//  If you're looking for the code to solve a specific problem, it starts down below, at about line 245
...
int get_min(vector<pair<int, int>> &tree, int v, int tl, int tr, int l, int r) {
...
void update(vector<pair<int, int>> &tree, int v, int tl, int tr, int l, int r, int value) {
    ...
    if (l == tl && tr == r) {
        tree[v].first = tree[v].second = value;
    } else {
        push_tree(tree, v);
        int tm = (tl + tr) / 2;
...
signed main() {
    ...
    for (int req = 0; req < m; ++req) {
        ...
        if (req_type == '1') {
            ...
            cin >> l >> r;
            cout << get_min(tree, 1, 0, n - 1, l, r - 1) << endl;
        }
    }
}
